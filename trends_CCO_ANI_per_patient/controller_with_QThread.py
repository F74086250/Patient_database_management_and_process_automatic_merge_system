...
# class WorkThread_combine_data(QThread):
class WorkThread_combine_data(QThread):
    # 自定義訊號物件。引數str就代表這個訊號可以傳一個字串
    trigger = pyqtSignal(str)

    def __init__(self,Trends_filename,CCO_filename,ANI_filename,CCO_time_gap,ANI_time_gap,SerialNum_003Num,patient_name,df_time_gap_record):
        # 初始化函式
        super(WorkThread_combine_data, self).__init__()
        self.Trends_filename=Trends_filename
        self.CCO_filename=CCO_filename
        self.ANI_filename=ANI_filename
        self.CCO_time_gap=CCO_time_gap
        self.ANI_time_gap=ANI_time_gap
        self.SerialNum_003Num=SerialNum_003Num
        self.patient_name=patient_name
        self.df_time_gap_record=df_time_gap_record
    ...
    def run(self):
        for i in range(len(self.Trends_filename)):
            ...
            CCO_df,CCO_df_without_time_gap_shift=self.CCO_processing(self.CCO_filename[i],self.CCO_time_gap[i],3)
            CCO_df.to_csv(f"./CCO_csv/{self.SerialNum_003Num[i]}_CCO.csv",index=False,encoding='utf-8-sig')
            CCO_df_without_time_gap_shift.to_csv(f"./CCO_without_time_gap_shift_csv/{self.SerialNum_003Num[i]}_CCO_without_time_gap_shift.csv",index=False,encoding='utf-8-sig')
    ...
    def CCO_processing(self,CCO_filename,CCO_time_gap,row_num):
        ...
        for i in range(len(CCO_filename)):
            ...
            df_without_time_gap_shift=df.copy(deep=True)
            for i in range(len(df_without_time_gap_shift['Time'])):
                if(len(df_without_time_gap_shift['Time'][i]))>8:
                    df_without_time_gap_shift['Time'][i]=df_without_time_gap_shift['Time'][i][2:]
                    df_without_time_gap_shift['Time'][i]="0"+df_without_time_gap_shift['Time'][i]
            df=self.time_gap_preprocessing(df,CCO_time_gap)
            df=self.data_avg(df,row_num)
            df_without_time_gap_shift=self.data_avg(df_without_time_gap_shift,row_num)
            CCO_df_list.append(df)

        if len(CCO_df_list)==1:
            return CCO_df_list[0],df_without_time_gap_shift
        else:
            df_vertical_merge=self.data_df_vertical_merge(CCO_df_list)
            return df_vertical_merge,df_without_time_gap_shift
    ...
    def CCO_remove_outliers(self,df):
        ...
        range_dic['平均脉搏速率(次/分)']=[40.0,100.0]
        range_dic['平均血压(mmHg)']=[70.0,120.0]
    ...
    def ANI_processing(self,ANI_filename,ANI_time_gap,row_num):
        ...
        for i in range(len(ANI_filename)):
            ...
            df= df.drop(1, axis = 1)
            df.columns=['Time','Energy','ANI','ANImean','Quality','Time_duplicated']
            time_duplicated=[]
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def start_click(self):
        ...
        self.work_thread_combine_data=WorkThread_combine_data(self.Trends_filename,self.CCO_filename,self.ANI_filename,self.CCO_time_gap,self.ANI_time_gap,self.SerialNum_003Num,self.patient_name,self.df_time_gap_record)
        self.work_thread_combine_data.start()
        return
